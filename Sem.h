...
int getTypeFromCalcule(char assignedValue[20]){
    int dot = 0;
    for(int i = 7; i > 0 && dot == 0;i--){
        if(assignValue[i]== '.') dot = 1;
        else{
            if(assignValue[i] != '0'){
                return 1;
            }
